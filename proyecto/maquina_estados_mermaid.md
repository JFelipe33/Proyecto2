...
stateDiagram-v2
    ...
    %% CORRECCIÓN: Desbloqueo directo a marcha mínima
    STATE_EMERGENCY_STOP --> STATE_BANDA_RUNNING : EVENT_BUTTON_PRESSED / Apagar LED, Velocidad = 70%, Motor FORWARD

    %% NOTA PARA EL DIAGRAMA
    note right of STATE_EMERGENCY_STOP
        El evento EVENT_EMERGENCY 
        solo se dispara si el botón físico se mantiene presionado
        continuamente durante un tiempo >= 2.0 segundos.
    end note
    %% NOTA PARA EL DIAGRAMA
